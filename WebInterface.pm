...
if (1 || Url::par_set("debug")) {SpotterHTMLUtil::activate_debugging_output()}
...
sub show_functions {
  ...
  if (!Url::par_is("login","form")) {
    if ($login->logged_in()) {
      ...
      $out = $out .  "not logged in<br>\n";
      $out = $out .  "(To check your answers to problems without being logged in, start from the book's web page.)<p>\n";
    }
  }
